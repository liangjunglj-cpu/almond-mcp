...
function evidence(a) {
  ...
  return `<details open><summary>Source & attribution</summary><p>${esc(a.publisher)}<br>${esc(a.license)}</p>${origin ? `<p><a href="${esc(origin)}" target="_blank" rel="noopener noreferrer">${a.kind === 'model' ? 'Generation provider' : 'Original source'} ↗</a></p>` : '<p>No external source link recorded.</p>'}${p ? `<dl>${dl('Generated', g.generated_at)}${dl('Mesh model',g.mesh_model)}${dl('Mesh task ID', g.mesh_task_id)}${dl('Task evidence',g.import_record?.task_id_status)}${dl('Image provider',p.image_generation?.provider)}${dl('Image task / job ID',p.image_generation?.id)}</dl><p>${esc(p.rights_evidence)}</p>` : '<p>Representation-only element. Check the original source terms for project use. Downloaded community files are local to this installation and are excluded from the distributable plugin.</p>'}</details>
    ...
    ${g.import_record ? `<details open><summary>Project import &amp; simplification</summary><p>${esc(g.import_record.project)}</p><p>${esc(g.import_record.intended_role)}</p><p>${esc(g.import_record.dimension_basis)}</p><p>${esc(g.import_record.conversion.material_derivation)}</p><dl>${dl('Original triangles',g.import_record.conversion.source_triangles)}${dl('Library triangles',a.metadata.triangle_count)}${dl('Source OBJ',g.import_record.conversion.source_file)}${dl('Source SHA-256',g.import_record.conversion.source_sha256)}</dl></details>` : ''}
...
async function load() {
  ...
  try {
    ...
    $('home-model-count').textContent=data.counts.models+' MODELS · LINKED DRAWINGS · SOURCE RECORDS';
